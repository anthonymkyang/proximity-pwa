...
export default function GroupPage() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        // 3) Attendees (only approved/accepted, newest first, limit 50)
        const { data: attendeeRows, error: attendeesErr } = await supabase
          ...
          .in("status", ["approved", "accepted"]) // only show approved/accepted in UI
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      ...
      // Remove current user from local attendee list and reset state
      setAttendees((prev) =>
              ...
              <>
                ...
                <div className="">
                  {attendeesPreview.length > 0 ? (
                    <div className="flex items-center gap-2">
                      <Avatar14
                        avatars={attendeesPreview.map((row) => {
                          const displayName =
